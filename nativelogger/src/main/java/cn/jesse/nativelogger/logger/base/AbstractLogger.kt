...
abstract class AbstractLogger(tag: String) : ILogger {
    ...
    override fun log(level: LoggerLevel, subTag: String, cause: Throwable) {
        when (level) {
            LoggerLevel.DEBUG -> debug(subTag, cause)
            LoggerLevel.INFO -> info(subTag, cause)
            LoggerLevel.WARN -> warn(subTag, cause)
            LoggerLevel.ERROR -> error(subTag, cause)
            else -> throw NLoggerError(ERROR_LEVEL)
        }
    }
